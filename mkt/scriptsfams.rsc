# feb/02/2016 17:24:24 by RouterOS 6.33.3
# software id = 68PD-NRBL
#
/system script
add name=startup owner=admin policy=\
    ftp,reboot,read,write,policy,test,password source=":delay 15;\
    \n\
    \n\
    \n:global WAN1 ether1-gvt\
    \n\
    \n:global WAN2 ether2-net\
    \n\
    \n\
    \n:global HOSTP1 8.8.4.4;\
    \n:global HOSTP2 131.0.103.44;\
    \n\
    \n:global GW1 0;\
    \n:global GW2 0;\
    \n\
    \n\
    \n:global myGW2 0;"
add name=FULLFAILOVER owner=admin policy=\
    ...
    \n\
    \n\
    \n:local myGW1 [/ip route get [/ip route find gateway=\$WAN1] dst];\
    \n:local myGW2 [/ip dhcp-client get value-name=gateway number=0];\
    \n\
    \n\
    \n:for i from=( [:len \$myGW1] - 1) to=0 do={ \
    \n\
    \n  :if ( [:pick \$myGW1 \$i] = \"/\") do={ \
    \n\
    \n    :set myGW1 [:pick \$myGW1 0 \$i];\
    \n\
    \n  }\
    \n\
    \n}\
    ...
    \n  :global GW1 \$myGW1\
    \n\
    \n    /ip route remove [find comment=\"DST_H1\"] \
    \n\
    \n    /ip route add dst-address=\$HOSTP1 gateway=\$GW1 scope=10 comment=\"\
    DST_H1\"\
    \n\
    \n    /ip route remove [find comment=\"DEF_GW1\"]\
    \n\
    \n    /ip route add dst-address=0.0.0.0/0 gateway=\$HOSTP1 scope=30 commen\
    t=\"DEF_GW1\" distance=1\
    \n\
    \n  }\
    \n\
    \n\
    ...
    \n:if ( \$GW2 != \$myGW2 ) do { \
    \n   :global GW2 \$myGW2\
    \n   /ip route remove [find comment=\"DST_H2\"] \
    \n   /ip route add dst-address=\$HOSTP2 gateway=\$GW2 scope=10 comment=\"D\
    ST_H2\"\
    \n   /ip route remove [find comment=\"DEF_GW2\"]\
    \n\
    \n   /ip route add dst-address=0.0.0.0/0 gateway=\$HOSTP2 scope=30 comment\
    =\"DEF_GW2\" distance=2\
    \n\
    \n}"
add name=bloqueia owner=admin policy=\
    ftp,reboot,read,write,policy,test,password,sniff,sensitive source=":global\
    \_blockip\
    \n/ip firewall address-list add address=\$blockip list=drop-traffic"
add name=monitorWAN1 owner=admin policy=\
    ftp,reboot,read,write,policy,test,password,sniff,sensitive source=":global\
    \_HOSTP1LOST\
    \n:global HOSTP1\
    \n:global WAN1\
    \n:set \$HOSTP1LOST [ /ping \$HOSTP1 interface=\$WAN1 count=10 interval=1 \
    ];"
add name=monitorWAN2 owner=admin policy=\
    ftp,reboot,read,write,policy,test,password,sniff,sensitive source=":global\
    \_HOSTP2LOST\
    \n:global HOSTP2\
    \n:global WAN2\
    \n:set \$HOSTP2LOST  [ /ping \$HOSTP2 interface=\$WAN2 count=10 interval=1\
    \_]"
